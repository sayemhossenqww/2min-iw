...
class PointOfSaleEdit extends Component<Props, State> {
    ...
    setupForEditing = async (): Promise<void> => {
        ...
        let gft = order.is_gift === 1 ? "gift" : "normal";

        this.setState({ gift: gft });
        this.setState({ is_gift: order.is_gift });
        ...
        console.log(this.state.is_gift);
    ...
    handleInvoiceTypeChange = (event: React.ChangeEvent<HTMLSelectElement>): void => {
        this.setState({ is_gift: parseInt(event.target.value) });
    };
            ...
            <React.Fragment>
                <div className="d-flex py-3">
                    ...
                    <select
                        ...
                        value={this.state.is_gift} // ✅ Controlled component
                        onChange={e => this.handleInvoiceTypeChange(e)}
                    >
                        <option value="0">{t('Normal Invoice', 'الفاتورة العادية')}</option>
                        <option value="1">{t('Gift Invoice', 'فاتورة الهدية')}</option>
                    </select>
