...
<body>
    ...
    <div style="margin-bottom: 0.2rem;text-align: center !important;">
        ...
        @if ($settings->storePhone)
            <div style="font-size: 1rem; font-weight: bold;">{{ $settings->storePhone }}</div>
        @endif
    ...
    <div style="margin-bottom: 1.5rem">
        @foreach ($order->order_details as $detail)
            ...
            <div style="display: flex">
                <div>{{ $detail->quantity }} X {{ $order->is_gift ? '*******' : $detail->view_price }}</div>
                <div style="margin-left: auto">{{ $order->is_gift ? '*******' : $detail->view_total }}</div>
            </div>
        @endforeach
    </div>
